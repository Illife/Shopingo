          ...
          <form action="<?= PATH ?>/handlers/products_read_by_category_id.php" method="post" class="form_products_read">
            <select class="form-select select_products_read" name="select">
              ...
              <?php foreach ($ResultReadCategories as $category) { ?>
                <option value="<?= $category['id'] ?>" class="form_products_read_option"><?= $category['title'] ?></option>
              <?php } ?>
            </select>
          </form>
        </div>
    ...
    <div class="card-body">
      ...
      <div class="table-responsive">
        <table class="table align-middle table-striped">
          <tbody>
            <?php foreach ($ResultReadProductsWithPagination as $product) { ?>
              <tr>
                <td>
                ...
                <td>
                  <div class="d-flex align-items-center gap-3 fs-6">
                    ...
                    <a href=<?= PATH . '/admin/product-delete/?id=' . $product['id'] ?>" class="text-danger" data-bs-toggle="tooltip" data-bs-placement="bottom" title="" data-bs-original-title="Delete" aria-label="Delete"><i class="bi bi-trash-fill"></i></a>
                  </div>
                </td>
              </tr>
            <?php } ?>
      ...
      <nav class="float-end mt-4" aria-label="Page navigation">
        <ul class="pagination">
          ...
          ?>
          <li class="page-item"><a class="page-link" href="<?= PATH . '/admin/products/' . '?page=' . $previous ?>">Назад</a></li>
          <?php for ($i = 1; $i <= $countPages; $i++) { ?>
            <li class="page-item <?php if ($i == $page) {
                                    echo ('active');
                                  } ?>"><a class="page-link" href="<?= PATH . '/admin/products/' . '?page=' . $i ?>"><?= $i ?></a></li>
          <?php } ?>
          <li class="page-item"><a class="page-link" href="<?= PATH . '/admin/products/' . '?page=' . $next ?>">Вперёд</a></li>
        </ul>
      </nav>
